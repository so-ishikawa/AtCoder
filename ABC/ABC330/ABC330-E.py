...
for i in task_list:
    index = i[0]
    new_value = i[1]
    old_value = A_list[index]
    # print(index, new_value, old_value)

    num_dic[old_value] -= 1
    if num_dic[old_value] == 0:
        mex_set.add(old_value)
    num_dic[new_value] += 1
    if new_value in mex_set:
        mex_set.discard(new_value)
